...
class AgentManager:
    ...
    def initialize(self):
        obj = self.bus.get_object(BLUEZ_SERVICE_NAME, "/org/bluez")
        manager = dbus.Interface(obj, "org.bluez.AgentManager1")

        path = "/test/agent"
        manager.RegisterAgent(path, self.capability)

        self.agent = Agent(self.bus, path)
        _LOGGER.info("edl: auto-pair/accept agent registered - new bluez 5.7 method")
...
#         _LOGGER.debug("Timed out. Cancelling pairing")
#         device_obj.CancelPairing()
#     else:
#         _LOGGER.debug("Creating device failed: %s" % (error))
